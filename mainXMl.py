...
XML_PATH = os.path.join(BASE_DIR, "ecommerce_data.xml")

if os.path.exists(XML_PATH):
    st.sidebar.success("✅ Loaded existing XML file.")
    df = pd.read_xml(XML_PATH)
else:
    st.sidebar.warning("⚠️ No XML file found — generating dummy data...")
    df = generate_dummy_ecommerce_data(1000000)
    df.to_xml(XML_PATH, index=False)
    st.sidebar.info("💾 Dummy data saved as XML for next run.")
